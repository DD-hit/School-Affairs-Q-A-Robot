...
function initPage() {
    ...
    userInfoElements.forEach(element => {
        if (mockData.currentUser) {
            element.innerHTML = `
                <span>${mockData.currentUser.username}</span>
                <div class="avatar">${mockData.currentUser.avatar}</div>
            `;
        }
    });
            ...
            if (submitBtn) {
                ...
                setTimeout(() => {
                    ...
                    if (form.id === 'login-form') {
                        ...
                        setTimeout(() => navigateTo('forum.html'), 1000);
...
function showNewPostModal() {
    window.location.href = 'post.html?mode=new';
}
